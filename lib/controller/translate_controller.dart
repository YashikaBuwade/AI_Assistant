...
//  Added this because you deleted image_controller.dart
...
class TranslateController extends GetxController {
  ...
  Future<void> translate() async {
    if (textC.text.trim().isNotEmpty && to.isNotEmpty) {
      ...
      try {
        final res = await APIs.getGeminiAnswer(prompt);
        // resultC.text = utf8.decode(res.codeUnits); // Optional: keep this if you had encoding issues

        print(' Gemini returned: $res');
        resultC.text = res;

        status.value = Status.complete;
      } catch (e) {
        status.value = Status.none;
        MyDialog.info('Failed to translate: $e');
